...
function findCaliforniaCafes(s) {
    ob={"name": '',
    "street_no": "",
    "locality": "",
    "postal_code": "",
...
cafes.map((item)=>{
   ...
   if(xx.includes(s)){
       ...
       places.map(item2=>{
        if(item2.id===y){
            ob.street_no=item2.street_no;
            ob.locality=item2.locality;
            ob.postal_code = item2.postal_code;
            ob.lat=item2.lat;
            ob.long = item2.long;
            console.log(ob);
        }
...
readline.question('Enter String: ', name => {
    ...
    findCaliforniaCafes(name.toLowerCase());
    readline.close();
  })